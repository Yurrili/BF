...
import com.uj.yuri.budgetflow.DataManagment.GatewayTemplate.IncomeGatewayTemplate;
...
public class IncomeManagment {
    ...
    private IncomeGatewayTemplate incomeGateway ;

    public IncomeManagment(Context ctx) {
        this.incomeGateway = new IncomeGatewayTemplate(ctx);
    }

    public IncomeGatewayTemplate getIncomeGateway() {
    ...
    public ArrayList<Income> selectAllIncomes(){
        ...
        if (c != null) {
            for (c.moveToFirst(); !c.isAfterLast(); c.moveToNext()) {
                Income inc = null;
                try {
                    inc = IdentityMap.IncomesIdentityMap.isInto(c.getString(0));
                    if ( inc == null) {
